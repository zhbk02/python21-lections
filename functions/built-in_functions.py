...
# lambda параметры: что функкция возвращает
add = lambda a, b: a + b
print(add(5, 4)) # 9

"map" # - это функкция которая принимает первым аргументом функцию, вторым последовательнось ( итерируемыйй) обьект. map возвращает генератор, в коттором все элементы - результат пиринимает функции, в которую мы передали элемент из последовательности
